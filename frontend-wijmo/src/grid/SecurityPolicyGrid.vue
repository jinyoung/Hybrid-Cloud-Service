<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['policyId','policyName','policyDescription','policyEnforcement','policyType',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="policyId" header="policyId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="policyName" header="policyName" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="policyDescription" header="policyDescription" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="policyEnforcement" header="policyEnforcement" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="policyType" header="policyType" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">SecurityPolicy 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <SecurityPolicy :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="securityPolicyModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
<script>
import SecurityPolicy from '../components/SecurityPolicy.vue'
...
export default {
    name : 'securityPolicy-grid',
    mixins:[SecurityPolicy],
    components:{
        SecurityPolicy,
    },
    data: () => ({
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.securityPolicy.href;
