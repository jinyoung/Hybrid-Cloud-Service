<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['ticketId','customerId','ticketSeverity','ticketDescription','ticketStatus',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="ticketId" header="ticketId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="customerId" header="customerId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="ticketSeverity" header="ticketSeverity" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="ticketDescription" header="ticketDescription" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="ticketStatus" header="ticketStatus" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">SupportTicket 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <SupportTicket :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="supportTicketModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
export default {
    name : 'supportTicket-grid',
    mixins:[SupportTicket],
    components:{
        SupportTicket,
    },
    data: () => ({
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.supportTicket.href;
