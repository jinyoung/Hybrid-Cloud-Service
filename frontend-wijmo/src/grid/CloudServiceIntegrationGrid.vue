<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['integrationId','integrationName','cloudServiceProvider','cloudServiceType','connectionString','integrationStatus',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="integrationId" header="integrationId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="integrationName" header="integrationName" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="cloudServiceProvider" header="cloudServiceProvider" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="cloudServiceType" header="cloudServiceType" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="connectionString" header="connectionString" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="integrationStatus" header="integrationStatus" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">CloudServiceIntegration 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <CloudServiceIntegration :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="cloudServiceIntegrationModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
<script>
import CloudServiceIntegration from '../components/CloudServiceIntegration.vue'
...
export default {
    name : 'cloudServiceIntegration-grid',
    mixins:[CloudServiceIntegration],
    components:{
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.cloudServiceIntegration.href;
