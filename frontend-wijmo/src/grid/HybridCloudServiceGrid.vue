<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['serviceId','serviceName','serviceProvider','serviceType','serviceDescription',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="serviceId" header="serviceId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="serviceName" header="serviceName" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="serviceProvider" header="serviceProvider" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="serviceType" header="serviceType" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="serviceDescription" header="serviceDescription" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">HybridCloudService 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <HybridCloudService :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="hybridCloudServiceModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
<script>
import HybridCloudService from '../components/HybridCloudService.vue'
...
export default {
    name : 'hybridCloudService-grid',
    mixins:[HybridCloudService],
    components:{
        HybridCloudService,
    },
    data: () => ({
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.hybridCloudService.href;
